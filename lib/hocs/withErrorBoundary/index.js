  ...
  return function (_Component) {
    ...
    _createClass(WithErrorBoundary, [{
      ...
      value: function componentDidCatch(error, errorInfo) {
        var customErrorReportService = this.props.customErrorReportService;

        this.setState({ hasError: true, error: error, errorInfo: errorInfo });

        var errorCallback = customErrorReportService || console.log;
        errorCallback(error, errorInfo, this.props);
      }
    }, {
      key: 'render',
      value: function render() {
        var FallbackComponent = this.props.customFallbackView || _FallbackView2.default;
